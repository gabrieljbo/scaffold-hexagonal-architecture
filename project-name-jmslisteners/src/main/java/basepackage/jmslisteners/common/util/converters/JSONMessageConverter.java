...
public class JSONMessageConverter<T> implements MessageConverter<T> {
	...
	try {
	    ...
	    String exceptionMessage = messages.getString(ERROR_CODE_PREFIX + MessageConverterErrorCode.CONVERSION_ERROR_CODE.getCode());
	    throw new SystemException(ex, MessageConverterErrorCode.CONVERSION_ERROR_CODE, exceptionMessage);
	}
